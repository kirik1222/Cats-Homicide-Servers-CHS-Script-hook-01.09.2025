...
if ( SERVER ) then
	...
	hook.Add( "PlayerSay", "cc_easteregg", function( ply, text, public )	
		...
		if string.sub( text:lower(), 1, 34 ) == "i think that black people are okay" then		
		
			if not ( ply:GetPData( "eastereggs1fix" ) and ply:GetPData( "eastereggs1fix" ) == "true" ) then	
			
				ply:SetPData( "eastereggs1fix", "true" )	
				
				if not ply:GetPData( "eastereggsfix" ) then
					ply:SetPData( "eastereggsfix", 1 )
				else
					ply:SetPData( "eastereggsfix", ply:GetPData( "eastereggsfix" ) + 1 )
				end		
				
				ULib.tsayColor( nil, false, white, "[CC] ", blue, ply:Nick(), white, " has found easter egg #1!" )		
				
				if ply:GetPData( "eastereggsfix" ) and ply:GetPData( "eastereggsfix" ) == "3" then
					ULib.tsayColor( nil, false, white, "[CC] ", blue, ply:Nick(), white, " has found all 3 easter eggs!" )
				elseif ply:GetPData( "eastereggsfix" ) and ply:GetPData( "eastereggsfix" ) ~= "3" then
					ULib.tsayColor( nil, false, white, "[CC] ", blue, ply:Nick(), white, " has found ", white, ply:GetPData( "eastereggsfix" ), white, "/3 easter eggs." )
				end
			...
			elseif ply:GetPData( "eastereggs1fix" ) and ply:GetPData( "eastereggs1fix" ) == "true" then		
	...
	concommand.Add( "42%", function( ply )
	
		if not ( ply:GetPData( "eastereggs2fix" ) and ply:GetPData( "eastereggs2fix" ) == "true" ) then	
		
			ply:SetPData( "eastereggs2fix", "true" )		
		...
		elseif ply:GetPData( "eastereggs2fix" ) and ply:GetPData( "eastereggs2fix" ) == "true" then	
	...
	net.Receive( "egg", function( len, ply )
		...
		if not ( ply:GetPData( "eastereggs3fix" ) and ply:GetPData( "eastereggs3fix" ) == "true" ) then	
		
			ply:SetPData( "eastereggs3fix", "true" )	
		...
		elseif ply:GetPData( "eastereggs3fix" ) then
		
			if ply:GetPData( "eastereggs3fix" ) and ply:GetPData( "eastereggs3fix" ) == "true" then
...
if ( CLIENT ) then
	...
	CreateConVar( "eastereggs_enable", "0",FCVAR_NONE,"Niggers are not..." )
	
	cvars.AddChangeCallback( "eastereggs_enable", function( cvar, oldValue, newValue )	
	
		if newValue == "human" then
			net.Start( "egg" )
			net.SendToServer()
		end		
	...
	concommand.Add( "hint_easteregg", function( ply )
		print("Trannies suicide percentage:")
	end)
end

function ulx.resetdata( calling_ply, target_ply )

	if target_ply:GetPData( "eastereggsfix" ) then
		target_ply:RemovePData( "eastereggsfix" )
	end
	
	if target_ply:GetPData( "eastereggs1fix" ) then
		target_ply:RemovePData( "eastereggs1fix" )
	end
	
	if target_ply:GetPData( "eastereggs2fix" ) then
		target_ply:RemovePData( "eastereggs2fix" )
	end
	
	if target_ply:GetPData( "eastereggs3fix" ) then
		target_ply:RemovePData( "eastereggs3fix" )
	end
