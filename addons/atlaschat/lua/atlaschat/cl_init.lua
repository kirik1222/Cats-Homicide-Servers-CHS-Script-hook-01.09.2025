...
hook.Add("player_disconnect", "atlaschat.DisconnectMessage", function(data)
	local filtered = atlaschat.filterJoinDisconnect:GetBool() or LocalPlayer().Syncing
...
hook.Add("OnPauseMenuShow","atlasnigger",function()
	local panel = atlaschat.theme.GetValue("panel")
...
net.Receive("atlaschat.nwpm", function(bits)
	...
	if ((LocalPlayer().IsAdmin)and not(LocalPlayer():IsAdmin()))then LocalPlayer():PrintMessage(HUD_PRINTTALK,"You are not an admin.") return end
...
net.Receive("atlaschat.gtplpm", function(bits)
	...
	if (!ValidPanel(chatRoom)) and not((LocalPlayer().IsAdmin)and not(LocalPlayer():IsAdmin()))then
	...
	for i = 1, numPlayers do
		...
		if (IsValid(player)) and chatRoom!=nil then
	...
	if (IsValid(creator)) and chatRoom!=nil then
