...
hook.Add("Move", "TFALeanThink", function(ply)
	...
	--[[if SERVER and math.abs(targ)==1 then
		if not(ply.CreatedBullseye) then
			ply.CreatedBullseye=true
			local bone=ply:LookupBone("ValveBiped.Bip01_Head1")
			if bone then
				ply.bullsEye=ents.Create("npc_bullseye")
				local pos,ang=ply:GetBonePosition(bone)
				pos=pos+ang:Forward()*5
				ply.bullsEye:SetPos(pos)
				ply.bullsEye:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE)
				ply.bullsEye:AddEFlags(bit.bor(1048576,131072))
				ply.bullsEye:SetKeyValue("Health","30000")
				ply.bullsEye:SetKeyValue("Minimum Angle","360")
				ply.bullsEye:SetKeyValue("Autoaim Radius","100000")
				ply.bullsEye:SetKeyValue("Minimum Distance","100000")
				ply.bullsEye:Spawn()
				ply.bullsEye:SetParent(ply)
				ply.bullsEye:Fire("SetParentAttachmentMaintainOffset", "anim_attachment_head", 0.1)
				ply.bullsEye:CallOnRemove("Huy",function(ent) ply:PrintMessage(HUD_PRINTTALK,"REMOVED") end)
				...
				for npcType,tabl in pairs(NPC_RELATIONSHIPS) do
					for class,bool in pairs(tabl) do
						for i,ent in pairs(ents.FindByClass(class)) do
							if (npcType=="Combine" and ply.Role!="combine") or (npcType=="Humans" and ply.Role=="combine") then
								ent:AddEntityRelationship(ply.bullsEye,D_HT,99)
								if ent:GetEnemy()==ply then ent:SetEnemy(ply.bullsEye) ent:UpdateEnemyMemory(ply.bullsEye,ply.bullsEye:GetPos()) end
							end
						end
					end
				end
	...
	elseif ply.bullsEye then
		ply.CreatedBullseye=nil
		if IsValid(ply.bullsEye) then ply.bullsEye:Remove() end
		ply.bullsEye=nil
	end]]
	if SERVER then
...
function LeanCalcView(ply, pos, angles, fov)
	...
	local off = Vector(0, status * -LeanOffset, 0)
	off:Rotate(angles)
	view.angles:RotateAroundAxis(view.angles:Forward(), status * 12)
...
hook.Add("CalcView", "TFALeanCalcView", function(ply, pos, angles, fov, ...)
	if not(ply:Alive()) then ply=GAMEMODE.Spectatee end
