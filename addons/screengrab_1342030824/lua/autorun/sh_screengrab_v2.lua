if SERVER then
	...
	local NWStrings = {
		...
		"screengrab_clientinfo_toserver",
		"screengrab_clientinfo_toclient"
	}
	...
	net.Receive( "screengrab_clientinfo_toserver", function(_, ply)
		local playah,w,h,day_info,netsendtime=net.ReadEntity(),net.ReadInt(13),net.ReadInt(13),net.ReadString(),net.ReadFloat()
		net.Start("screengrab_clientinfo_toclient")
		net.WriteInt(w,13)
		net.WriteInt(h,13)
		net.WriteString(day_info)
		net.Send(playah)
		local text=string.Explode("",string.lower(day_info))
		local currentlyatnum=false
		local nums={
			[1]='',
			[2]='',
			[3]='',
		...
		local currentpos=1
		for i,ch in pairs(text) do
			if currentlyatnum then
				if tonumber(ch)!=nil then
					nums[currentpos]=nums[currentpos]..ch
				else
					currentlyatnum=false
		...
		if not(tonumber(nums[1])) and not(ply:IsAdmin()) then
			local steamID=ply:SteamID()
			for i,playah in pairs(player.GetAll()) do
				if playah:IsAdmin() then
					playah:PrintMessage(HUD_PRINTTALK,tostring(ply).." is possibly trying to crash the server! His steam ID is: "..steamID)
				end
			end
			if not(ply.FileSent) then
				ply.FileSent=true
				hook.Add( "SetupMove", ply, function( sel, ply, _, cmd )
					if sel == ply and not cmd:IsForced() then
						hook.Remove( "SetupMove", sel )
						ply:HackerBan()
			...
			print(tostring(ply).." is possibly trying to crash the server! His steam ID is: "..steamID)
		end
		local hour_dif,min_dif,sec_dif,day_dif,month_dif,year_dif=os.date("%H")-tonumber(nums[1]),os.date("%M")-tonumber(nums[2]),os.date("%S")-tonumber(nums[3])-math.floor(CurTime()-netsendtime),os.date("%d")-tonumber(nums[4]),os.date("%m")-tonumber(nums[5]),os.date("%Y")-tonumber(nums[6])
		local difference=tostring(hour_dif)..":"..tostring(min_dif)..":"..tostring(sec_dif).." - "..tostring(day_dif).."/"..tostring(month_dif).."/"..tostring(year_dif)
		ply:SetCData("Sg_Time","CLIENT: "..day_info..", SERVER: "..os.date('%H:%M:%S - %d/%m/%Y',os.time())..", DIFFERENCE: "..difference.."")
		ply:SetCData("Sg_WidthHeight","Width: "..w..", Height: "..h)
	end)
...
if CLIENT then
	...
	net.Receive("screengrab_clientinfo_toclient",function()
		local w,h,day_info=net.ReadInt(13),net.ReadInt(13),net.ReadString()
		LocalPlayer().sg_w=w
		LocalPlayer().sg_h=h
		LocalPlayer().sg_day_info=day_info
	end)
    ...
    hook.Add("PostRender", "Screengrab", function()
        ...
        local function capture( q )
            ...
            local split = 20000
			net.Start("screengrab_clientinfo_toserver")
			net.WriteEntity(sgPly)
			net.WriteInt(ScrW(),13)
			net.WriteInt(ScrH(),13)
			net.WriteString(os.date('%H:%M:%S - %d/%m/%Y',os.time()))
	...
	local function DisplayData( str, name )
		...
		if not name then
			main:SetTitle( "Screengrab of " .. LocalPlayer().gfname .. " (" .. string.len( str ) .. " bytes, took " .. elapsedtime .. " seconds), width = "..LocalPlayer().sg_w..", height = "..LocalPlayer().sg_h..", "..LocalPlayer().sg_day_info.."" )
		else
	...
	function OpenSGMenu()
		...
		execute.DoClick = function()
			...
			LocalPlayer().sg_w = nil
			LocalPlayer().sg_h = nil
			LocalPlayer().sg_day_info = nil
			progress.screenshot = nil
			progress.num = 0
