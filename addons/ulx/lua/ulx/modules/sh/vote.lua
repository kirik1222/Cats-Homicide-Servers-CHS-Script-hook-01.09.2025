...
local modes_simplified={}

for i,list in pairs(modes) do
	table.insert(modes_simplified,i)
	for name,val in pairs(list) do
		table.insert(modes_simplified,name)
	end
end
...
local function modeVoteFinished(t)
	local results = t.results
	local winner
	local winnernum = 0
	for id, numvotes in pairs( results ) do
	...
	ulx.logString( str )
	Msg( str .. "\n" )
	local word=string.Explode(" ",t.title)
	word=word[#word]
	word=string.sub(word,1,string.len(word)-1)
	if t.options[winner] and t.options[winner]=="Yes" then
		for i,list in pairs(modes) do
			if list[word] then
				local mainModeNeeded=mode_fixes[i] or i
				if GAMEMODE.MainMode!=mainModeNeeded then
					RunConsoleCommand("hmcd_setmainmode",mainModeNeeded)
				end
				RunConsoleCommand("hmcd_setmode",mode_fixes[word] or word)
			elseif word==i then
				RunConsoleCommand("hmcd_setmainmode",mode_fixes[i] or i)
				RunConsoleCommand("hmcd_setmode","Normal")
			end
		end
	end
end

function ulx.voteMode( calling_ply, title )
	if ulx.voteInProgress then
		ULib.tsayError( calling_ply, "There is already a vote in progress. Please wait for the current one to end.", true )
		return
	end
	title="Set next round mode to "..title.."?"
	ulx.doVote( title, { "Yes", "No" }, modeVoteFinished )
	ulx.fancyLogAdmin( calling_ply, "#A started a vote (#s)", title )
end

local vote_mode = ulx.command( CATEGORY_NAME, "ulx votemode", ulx.voteMode, "!votemode" )
vote_mode:addParam{ type=ULib.cmds.StringArg, completes=modes_simplified, hint="mode", error="mode doesnt exist", ULib.cmds.restrictToCompletes, default="Deathmatch", ULib.cmds.optional }
vote_mode:defaultAccess( ULib.ACCESS_ADMIN )
vote_mode:help( "Starts a public vote for a gamemode." )
...
local function voteMapDone2( t, changeTo, ply )
	...
	if shouldChange then
		--file.Write("homicide_stats.txt",util.TableToJSON(GAMEMODE.PlayerStats))
