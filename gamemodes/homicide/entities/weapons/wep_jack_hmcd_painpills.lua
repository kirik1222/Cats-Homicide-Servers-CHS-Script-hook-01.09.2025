...
function SWEP:SecondaryAttack(reload)
	...
	if IsValid(Dude) and Dude:GetClass()=="prop_ragdoll" then
		self.Owner:SetAnimation(PLAYER_ATTACK1)
		local RagOwner=Dude:GetRagdollOwner()
		if(SERVER)then
			sound.Play("snd_jack_hmcd_pillsuse.wav",Dude:GetPos(),60,math.random(90,110))
			self.Owner:ViewPunch(Angle(-5,0,0))
			if IsValid(RagOwner) and RagOwner:Alive() then
				if not(RagOwner.StomachContents["Painkillers"]) then RagOwner.StomachContents["Painkillers"]={} end
				table.insert(RagOwner.StomachContents["Painkillers"],120)
				if(self.Poisoned)then HMCD_Poison(RagOwner,self.Poisoner,"CyanidePowder") end
			end
			self:Remove()
		end
	end
