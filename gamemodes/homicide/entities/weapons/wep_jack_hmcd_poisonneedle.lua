...
function SWEP:AttackFront()
	...
	if((IsValid(Ent))and(Ent:IsPlayer()))then
		if(self:CanBackStab(Ent))then
			...
			HMCD_Poison(Ent,self.Owner,"Tetrodotoxin")
			self:Remove()
		else
	...
	elseif ((IsValid(Ent))and(Ent:GetClass()=="prop_ragdoll") and (Ent.fleshy)) then
		...
		if(CanStab)then
			...
			sound.Play("snd_jack_hmcd_needleprick.wav",HitPos,40,math.random(90,110))
			self.Owner:ViewPunch(Angle(1,0,0))
			if IsValid(Ent:GetRagdollOwner()) and (Ent:GetRagdollOwner():Alive()) then
				Ent:GetRagdollOwner():ViewPunch(Angle(-.05,0,0))
				HMCD_Poison(Ent:GetRagdollOwner(),self.Owner,"Tetrodotoxin")
			end
			self:Remove()
		else
			self.Owner:PrintMessage(HUD_PRINTCENTER,"Must be behind!")
