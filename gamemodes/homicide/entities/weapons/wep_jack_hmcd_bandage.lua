...
function SWEP:PrimaryAttack()
	...
	if(SERVER)then
		...
		if(self.Owner.Bleedout<=0 and not(hasBacteria))then return end
		...
		if self.Owner.ZombieBacteria and not(self.Infected) then
			self.Owner:RemoveBacteria(60)
		end
		if self.Infected then
			self.Owner:AddBacteria(35,CurTime()-101)
		end
		self:SetNextPrimaryFire(CurTime()+1)
...
function SWEP:SecondaryAttack(reload)
	...
	if(SERVER)then
		...
		local Battlepass = false
		if not(IsValid(Dude)) then return end
		if IsValid(Dude:GetRagdollOwner()) then Battlepass=true Dude=Dude:GetRagdollOwner() end
		if (Dude:IsPlayer()) then Battlepass=true end
		local hasBacteria=false
		...
		if((IsValid(Dude))and(Battlepass)and(Dude.Bleedout>0 or hasBacteria))then
			...
			if Dude.ZombieBacteria and not(self.Infected) then
				Dude:RemoveBacteria(60)
			end
			if self.Infected then
				Dude:AddBacteria(35,CurTime()-101)
			end
			self:SetNextPrimaryFire(CurTime()+1)
			self:SetNextSecondaryFire(CurTime()+1)
			Dude:RemoveAllDecals()
			if self.Poisoned then HMCD_Poison(Dude,self.Owner) end
			self:Remove()
		end
	end
end
