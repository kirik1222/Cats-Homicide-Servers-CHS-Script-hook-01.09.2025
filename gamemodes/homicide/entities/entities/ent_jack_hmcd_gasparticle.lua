...
function ENT:Think()
	...
	for key,obj in pairs(ents.FindInSphere(SelfPos,200*self.Repulsion))do
		if(not(obj==self)and(self:Visible(obj)))then
			local chance=300
			if obj.HoldingBreath then chance=600 end
			if(obj.HmcdGas)then
				local Vec=(obj:GetPos()-SelfPos):GetNormalized()
				Force=Force-Vec*self.Repulsion*2
			elseif((obj:IsPlayer())and(obj:Team()==2)and(obj:Alive()and not(obj:GetNWString("Mask")=="Gas Mask" or obj.Role=="combine"))and(math.random(1,chance)==42))then
				HMCD_Poison(obj,self.Owner,"Cyanide")--obj:TakeDamage(1,nil,nil)
			end
		end
	end
