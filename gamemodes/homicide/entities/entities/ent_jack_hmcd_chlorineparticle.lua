...
function ENT:Think()
	...
	for key,obj in pairs(ents.FindInSphere(SelfPos,200*self.Repulsion))do
		if(not(obj==self)and(self:Visible(obj)))then
			...
			elseif((obj:IsPlayer())and(obj:Team()==2)and(obj:Alive()and not(obj:GetNWString("Mask")=="Gas Mask" or obj.Role=="combine"))and(math.random(1,5)==5))then
				...
				if not(obj.StomachContents["Chlorine"]) then obj.StomachContents["Chlorine"]={} end
				if not(obj.StomachContents["Chlorine"][1]) then obj.StomachContents["Chlorine"][1]=0 end
				local dose=2
				if obj.HoldingBreath then dose=1 end
				obj.StomachContents["Chlorine"][1]=obj.StomachContents["Chlorine"][1]+dose
				local Dmg=DamageInfo()
				Dmg:SetDamage(1)
				Dmg:SetDamageType(DMG_SNIPER)
				...
				if obj.fake then obj:SetHealth(obj:Health() - 1) end
				if chlorineCount>25 then obj:ApplyPain(150) end
			end
		end
	end
