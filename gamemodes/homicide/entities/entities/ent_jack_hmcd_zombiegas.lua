...
function ENT:Think()
	...
	for key,obj in pairs(ents.FindInSphere(SelfPos,200*self.Repulsion))do
		if(not(obj==self)and(self:Visible(obj)))then
			...
			elseif(obj:IsPlayer())and(obj:Team()==2)and(obj:Alive()and(obj:GetNWString("Mask")!="Gas Mask"))and not(GAMEMODE.Mode=="Zombie" and obj.Role=="killer")then
				...
				if math.abs(headpos.y-obj:GetPos().y)<1 then
					...
					if obj:GetNWString("Helmet")=="Motorcycle" then infectchance=infectchance*3 end
