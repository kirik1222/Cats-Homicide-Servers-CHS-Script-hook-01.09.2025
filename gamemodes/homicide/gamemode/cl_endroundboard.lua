...
local endMessages={
	[1]={
		text=function(mvp)
			if(GAMEMODE.MainMode=="SOE")then
				return Translate("traitor_win")
			else
				return Translate("murderer_win")
			end
		end,
	...
	[2]={
		text="innocent_win",
		color=Color(20, 120, 255)
	},
	[3]={
		text=function(mvp)
			return mvp:GetNWString("bystanderName").." "..Translate("wins")
		end,
		color=Color(200, 200, 200)
	},
	[4]={
		text="rebels_win",
		color=Color(51, 102, 0)
	},
	[5]={
		text="combine_win",
	...
	[6]={
		text=function(mvp)
			if GAMEMODE.Mode!="Strange" then
				return Translate("everyone_died")
			else
				return ""
			end
		end,
	...
	[10]={
		text=function(mvp)
			if(GAMEMODE.Mode=="Zombie")then
				return Translate("alpha_giveup")
			elseif(GAMEMODE.MainMode=="SOE")then
				return Translate("traitor_giveup")
			else
				return Translate("murderer_giveup")
			end
		end,
...
function GM:DisplayEndRoundBoard(data)
	...
	if IsValid(Dude)then Showin=true end
	...
	local info=endMessages[data.reason]
	if info then
		if isfunction(info.text) then
			winner:SetText(info.text(Dude))
		else
			winner:SetText(Translate(info.text))
		end
		winner:SetTextColor(info.color)
	end
	...
	if data.murdererName!="?" then
		...
		local msgs={}
		if(self.MainMode=="SOE")then
			msgs.text=Translate("the_traitor_was")
			msgs.color=Color(col.x * 255, col.y * 255, col.z * 255)
		else
			msgs.text=Translate("the_murderer_was")
			msgs.color=Color(col.x * 255, col.y * 255, col.z * 255)
		end
	...
	for k,v in pairs(team.GetPlayers(2)) do
		if not((LocalPlayer().Lost and v!=LocalPlayer()) or v.Hidden) then
	...
	if(Showin)then
		...
		if Dude then
			...
			Bottom:SetTitle("                         MVP: "..Dude:GetNWString("bystanderName"))
			Bottom:MakePopup()
