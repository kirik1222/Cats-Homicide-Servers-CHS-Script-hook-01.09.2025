...
local function addPlayerItem(self, mlist, ply, pteam)
	...
	function but:Paint(w, h)
		if not(LocalPlayer().Lost and ply!=LocalPlayer()) then
...
function GM:DoScoreboardActionPopup(ply)
	...
	if ply != LocalPlayer() then
		local t = Translate("sb_mute")
		if ply:IsMuted() then
			t = Translate("sb_unmute")
		end
	...
	--[[local stats = actions:AddOption( Translate("sb_stats") )
	stats:SetIcon("icon16/chart_bar.png")
	function stats:DoClick()
		if IsValid(ply) then
			net.Start("hmcd_stats_toserver")
...
local function doPlayerItems(self, mlist, pteam)
	...
	if del then
		timer.Simple(0, function() if IsValid(mlist) then mlist:GetCanvas():InvalidateLayout() end end)
	end
end
...
local function makeTeamList(parent, pteam)
	...
	but:Dock(RIGHT)
	but:SetText(Translate("sb_jointeam"))
...
function GM:ScoreboardShow()
	if IsValid(menu) then
		...
		local lab = Label("by Cat and Zac90. Version " .. tostring(GAMEMODE.Version or "error"), menu.Credits)
		lab:Dock(RIGHT)
...
function GM:HUDDrawScoreBoard()
	if self.Mode=="CS" and IsValid(menu) then
		surface.SetDrawColor(Color(40,40,40,255))
		local x,y=menu:GetPos()
		local ctWidth,ctHeight=menu:GetWide()/8,menu:GetTall()/10
		surface.SetFont("MersRadialSmall")
		local letterWidth,letterHeight=surface.GetTextSize("CT")
		surface.DrawRect(x+ctWidth/2, y-ctHeight+1, ctWidth, ctHeight)
		draw.SimpleText( "CT", "MersRadialSmall", x+ctWidth, y-ctHeight, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
		draw.SimpleText( self.CSVictories["CT"], "MersRadialSmall", x+ctWidth, y-ctHeight+letterHeight, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
		
		surface.DrawRect(x+menu:GetWide()-ctWidth*1.5, y-ctHeight+1, ctWidth, ctHeight)
		draw.SimpleText( "T", "MersRadialSmall", x+menu:GetWide()-ctWidth, y-ctHeight, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
		draw.SimpleText( self.CSVictories["T"], "MersRadialSmall", x+menu:GetWide()-ctWidth, y-ctHeight+letterHeight, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
		
		surface.DrawRect(x+menu:GetWide()/2-ctWidth, y-ctHeight+1, ctWidth*2, ctHeight)
		draw.SimpleText( "Round", "MersRadialSmall", x+menu:GetWide()/2, y-ctHeight, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
		draw.SimpleText( self.CSVictories["Round"], "MersRadialSmall", x+menu:GetWide()/2, y-ctHeight+letterHeight, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
	end
end