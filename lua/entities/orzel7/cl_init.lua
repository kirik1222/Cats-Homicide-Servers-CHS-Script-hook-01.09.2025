--DO NOT EDIT OR REUPLOAD THIS FILE OR ILL NOT GIVE YOU A HEAD
	...
	if self.nextEFX < CurTime() then
		...
		if emitter then
			for _, v in pairs( Pos ) do
				...
				particle:SetEndSize( math.Rand(0,100) )
...
function ENT:EngineActiveChanged( bActive )
	if bActive then
		self.ENG = CreateSound( self, "ORZEL_ENGINE" )
		self.ENG:PlayEx(0,0)
		
		self.DIST = CreateSound( self, "ORZEL_DIST" )
...
function ENT:Draw()
	...
	local Size = 200 + (self:GetRPM() / self:GetLimitRPM()) * 15 + Boost

	render.SetMaterial( mat )
	render.DrawSprite( self:LocalToWorld( Vector(-250,0,0) ), Size, Size, Color( 255, 150, 0, 255) )
